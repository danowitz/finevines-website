...
const cards = report.rows.map((row) => {
  const image = row.ok && row.file
    ? relative(dirname(resolve(output)), resolve(row.file)).replaceAll('\\', '/')
    : '';
...
</style></head><body><header><h1>${report.attempted}-wine image recovery canary</h1><p>${report.accepted}/${report.attempted} accepted &middot; ${recovered} prior misses recovered &middot; ${report.labelBatches} nano label batches &middot; catalog and ledger unchanged</p></header><main class="grid">${cards}</main></body></html>`;
