...
</style></head><body><header><h1>${report.attempted}-wine image recovery canary</h1><p>${report.accepted}/${report.attempted} accepted · ${recovered} prior misses recovered · ${report.labelBatches} nano label batches · click any candidate to enlarge</p></header><main class="wine-list">${cards.join('\n')}</main>
