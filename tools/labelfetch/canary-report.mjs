...
</style></head><body><header><h1>${report.attempted}-wine image comparison</h1><p>${report.accepted}/${report.attempted} accepted · ${recovered} prior misses recovered · ${report.labelBatches} ${esc(labelReader)} label batches · click any candidate to enlarge</p></header><main class="wine-list">${cards.join('\n')}</main>
