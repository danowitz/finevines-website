...
// quotes, producer-copy-only, one page where the source itself repeats a
// line as both unquoted producer voice and a standalone quoted pull-quote,
// and — now that bucket 4 exists — several tastingNote-only or
// tastingNote-plus-facts wines, since that is the largest bucket and the one
// a human most needs to judge. #1 below is the literal wine the fourth
// bucket was named after ("This vineyard sits next to Musigny...").
const SAMPLE_SKUS = [
  '712790', '603736*', '381020', '520581', '410440',
  '210663', '603742*', '510426', '212566', '470326',
];
...
function tastingNoteBlock(tastingNote) {
  if (!tastingNote.length) return '<p class="empty">No third-person editorial prose found on this page.</p>';
  return tastingNote.map((t) => `<p class="tasting-note">${esc(t)}</p>`).join('');
}
  ...
  <section class="card">
    ...
    <div class="columns">
      ...
      <div class="col extracted">
        ...
        <h4>4. Tasting note (third-person editorial prose)</h4>
        ${tastingNoteBlock(e.tastingNote)}
      </div>
    </div>
  </section>`;
}).join('\n');
...
<style>
  ...
  p.tasting-note { margin: 0 0 .75rem; padding-left: .75rem; border-left: 3px solid #999; }
...
facts, the producer/importer's own marketing voice, quoted third-party tasting notes, and third-person editorial
prose about the wine or its site (bucket 4 — added after the first pass showed this was the largest category,
not leftover filler). The left column is the catalog's current AI-generated copy; the right column is what this
extraction step proposes pulling forward. Nothing here has been written into <code>data/wines.json</code> — this
page is for deciding what, if anything, to adopt.</p>
${cards}
