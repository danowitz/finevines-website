...
func Delist(existing []model.Wine, roster []salesforce.WineRaw, eligibleIDs map[string]bool, now time.Time) (unavailable []model.Wine, drops map[string]string) {
	...
	for _, w := range existing {
		...
		// Out of stock but otherwise showable? Merchandisable is Eligible
		// minus the stock floor, so this stays correct however the stock
		// clause evolves (ceiled quantities, whole-bottle math, …).
		stockOnly := inRoster && Merchandisable(raw)
		if !stockOnly {
			drop(w)
			continue
		}
