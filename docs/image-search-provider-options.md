...
Status: implemented, with the superseded Google integrations retired on
2026-08-16.
...
Google Cloud Vision Web Detection was enabled in CI; it was not a
...
and recovered zero wines. The adapter and its credential were therefore removed.

Google Custom Search was also retired. It had contributed useful historical
images, which remain in the catalog with their source provenance, but the
current combined provider replaced it and Google has announced the API's
2027-01-01 discontinuation. Neither retired service is an active dependency.
...
| **Google Custom Search JSON API** | `searchType=image` returned image and source-page metadata. Results differed from browser Google Images. | Historically 100 queries/day free, then $5 per 1,000. | Closed to new customers and scheduled to discontinue on **2027-01-01**. | **Retired 2026-08-16.** Historical images and provenance remain; code and credentials do not. |
...
## Retired reverse-image experiment
...
It was evaluated as a later-stage seed-expansion tool. It could not replace the
initial text-to-image search because it required an input image.

The adapter submitted both the whole bottle and label crop, retained top-level
exact matches as provisional when no source page could be paired, and kept
merely similar images untrusted. The corrected adapter still produced zero
incremental recoveries in the frozen test, so it was retired on 2026-08-16.
...
4. protected human review for the remainder.
