...
export const ACTION_KINDS = new Set(['image-select', 'no-image', 'reviewer-image', 'image-reopen']);
...
export function protectedHeaders(extra = {}) {
  return {
    ...
    'Content-Security-Policy': "default-src 'self'; img-src 'self' data: blob: https://finevines.com; frame-ancestors 'none'; base-uri 'none'; form-action 'self'",
...
export function validateAction(input, context) {
  ...
  if (kind === 'image-reopen' && context.allowImageReopen !== true) throw new Error('invalid kind');
  const action = {
    schemaVersion: 1,
    id: context.id,
    ...
    candidateId: input.candidateId === '' && (kind === 'no-image' || kind === 'reviewer-image' || kind === 'image-reopen') ? '' : text('candidateId', 120, /^[A-Za-z0-9._-]+$/),
